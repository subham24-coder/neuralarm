     THUMB
     AREA   appcode, CODE, READONLY
     EXPORT __main
	 IMPORT printAnd
	 IMPORT printOr
	 IMPORT printNand
	 IMPORT printNor
	 IMPORT printNot
     IMPORT printMsg4p
	   ENTRY 
__main  FUNCTION
	
;giving inputs to each gate and then loading them into fp registers
LOGIC_AND1	BL printAnd
			VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.2
			VLDR.F32 S1,=0.2
			VLDR.F32 S3,=-0.2
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_AND2	VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.2
			VLDR.F32 S1,=0.2
			VLDR.F32 S3,=-0.2
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_AND3	VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.2
			VLDR.F32 S1,=0.2
			VLDR.F32 S3,=-0.2
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_AND4	VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.2
			VLDR.F32 S1,=0.2
			VLDR.F32 S3,=-0.2
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_OR1	BL printOr
			VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.7
			VLDR.F32 S1,=0.7
			VLDR.F32 S3,=-0.1
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_OR2	VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.7
			VLDR.F32 S1,=0.7
			VLDR.F32 S3,=-0.1
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_OR4	VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.7
			VLDR.F32 S1,=0.7
			VLDR.F32 S3,=-0.1
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_OR4	VLDR.F32 S0,=-0.1
			VLDR.F32 S1,=0.7
			VLDR.F32 S1,=0.7
			VLDR.F32 S3,=-0.1
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG

LOGIC_NOT1	BL printNot
			VLDR.F32 S0,=0.5
			VLDR.F32 S1,=0
			VLDR.F32 S1,=-0.7
			VLDR.F32 S3,=0.1
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_NOT2	VLDR.F32 S0,=0.5
			VLDR.F32 S1,=0
			VLDR.F32 S1,=-0.7
			VLDR.F32 S3,=0.1
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG

LOGIC_NAND1	BL printNand
			VLDR.F32 S0,=0.6
			VLDR.F32 S1,=-0.8
			VLDR.F32 S1,=-0.8
			VLDR.F32 S3,=0.3
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_NAND2	VLDR.F32 S0,=0.6
			VLDR.F32 S1,=-0.8
			VLDR.F32 S1,=-0.8
			VLDR.F32 S3,=0.3
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_NAND3	VLDR.F32 S0,=0.6
			VLDR.F32 S1,=-0.8
			VLDR.F32 S1,=-0.8
			VLDR.F32 S3,=0.3
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG
			
LOGIC_NAND4	VLDR.F32 S0,=0.6
			VLDR.F32 S1,=-0.8
			VLDR.F32 S1,=-0.8
			VLDR.F32 S3,=0.3
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG

LOGIC_NOR1	BL printNor
			VLDR.F32 S0,=0.5
			VLDR.F32 S1,=-0.7
			VLDR.F32 S1,=-0.7
			VLDR.F32 S3,=0.1
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG

LOGIC_NOR2	VLDR.F32 S0,=0.5
			VLDR.F32 S1,=-0.7
			VLDR.F32 S1,=-0.7
			VLDR.F32 S3,=0.1
			MOV R0,#1		; a0
			MOV R1,#0		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG

LOGIC_NOR4	VLDR.F32 S0,=0.5
			VLDR.F32 S1,=-0.7
			VLDR.F32 S1,=-0.7
			VLDR.F32 S3,=0.1
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#0		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG

LOGIC_NOR4	VLDR.F32 S0,=0.5
			VLDR.F32 S1,=-0.7
			VLDR.F32 S1,=-0.7
			VLDR.F32 S3,=0.1
			MOV R0,#1		; a0
			MOV R1,#1		; a1
			MOV R2,#1		; a2
			VMOV.F32 S3,R0
			VCVT.F32.S32 S3,S3
			VMOV.F32 S5,R1
			VCVT.F32.S32 S5,S5
			VMOV.F32 S6,R2
			VCVT.F32.S32 S6,S6
			B EXP_CALC
			LTORG


EXP_CALC	VMUL.F32 S15, S0, S3
			VMOV.F32 S8, S15
			VMUL.F32 S16, S1, S5
			VADD.F32 S8, S8, S16
			VMUL.F32 S17, S1, S6
			VADD.F32 S8, S8, S17
			VADD.F32 S8, S8, S3		; S8 stores the result a0*W0 + a1*W1 + a2*W2 + Bias
			B INIT_
							
INIT_		VMOV.F32 S11, #20		; No of iterations for (e^x) expansion
			VMOV.F32 S12, #1		; count
			VMOV.F32 S13, #1		; tmp
			VMOV.F32 S14, #1		; result = 1 initially
			VMOV.F32 S16, #1		; reg to hold 1
			VMOV.F32 S19, #1			
			
Loop 		VCMP.F32 S11, S12		; comparision to check no of iterations completed
			VMRS.F32 APSR_nzcv,FPSCR	; copy fpscr to apsr
			BLT NXT;
			VDIV.F32 S15, S8, S12	; tmp1=x/count
			VMUL.F32 S13, S13, S15	; tmp=tmp*tmp1;
			VADD.F32 S14, S14, S13	; result=result+tmp;
			VADD.F32 S12, S12, S16	; count++
			B Loop;
			
NXT	 		VADD.F32 S17,S14,S19	; (1+e^x)
			VDIV.F32 S18,S14,S17	;  g(x) = 1/(1+e^-x) == (e^x)/(1+e^x)
			B OUTPUT;
	 
OUTPUT 	VLDR.F32 S10,=0.5
		VCMP.F32 S18,S10
		VMRS.F32 APSR_nzcv,FPSCR	; Transfer floating-point flags to the APSR flags
		ITE HI						; if g(x) > S10 , print 1 else print 0
		MOVHI R4,#1
		MOVLS R4,#0		
		BL printMsg4p
		ADD R4,R4,#1;	
		CMP R4,#1
		BEQ LOGIC_AND2
		CMP R4,#2
		BEQ LOGIC_AND3
		CMP R4,#3
		BEQ LOGIC_AND4
		CMP R4,#4
		BEQ LOGIC_OR1
		CMP R4,#5
		BEQ LOGIC_OR2
		CMP R4,#6
		BEQ LOGIC_OR4
		CMP R4,#7
		BEQ LOGIC_OR4
		CMP R4,#8
		BEQ LOGIC_NOT1
		CMP R4,#9
		BEQ LOGIC_NOT2
		CMP R4,#10
		BEQ LOGIC_NAND1
		CMP R4,#11
		BEQ LOGIC_NAND2
		CMP R4,#12
		BEQ LOGIC_NAND3
		CMP R4,#13
		BEQ LOGIC_NAND4
		CMP R4,#14
		BEQ LOGIC_NOR1
		CMP R4,#15
		BEQ LOGIC_NOR2
		CMP R4,#16
		BEQ LOGIC_NOR4
		CMP R4,#17
		BEQ LOGIC_NOR4

		
stop B stop ; stop program
	 ENDFUNC
	 END
